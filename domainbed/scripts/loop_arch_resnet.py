        ...
        #     "SpawriousM2M_hard",
                ...
                for mix_strategy in mix_strategy_list:
                        for mix_interpolation in ["CutMix", "Mixup"]:
                                ...
                                os.system(
                                f"""python3 -m domainbed.scripts.train_n --data_dir={data_dir}  --algorithm {algo} --test_env 0 --dataset {dataset} --hparams='{hparams}' --seed {args.seed} --output_dir {algo}-results --n_iter 2 --mix_strategy {mix_strategy} --mix_interpolation {mix_interpolation}"""
                                )
                                #   --pretrained_model_path {jtt_path}
