...
if __name__ == "__main__":
    ...
    for iteration in range(args.n_iter):
        ...
        for env_i, env in enumerate(dataset):
            ...
            out, in_ = misc.split_dataset(env,
                ...
                misc.seed_hash(args.trial_seed+iteration, env_i))
    ...
    print()
    average_results = {
        key: np.mean([best_list[j][key] for j in range(len(best_list))]) for key in last_results_keys
    }
    std_results = {
        key: np.std([best_list[j][key] for j in range(len(best_list))]) for key in last_results_keys
    }

    print("FINAL")
    misc.print_row(last_results_keys, colwidth=12)
    misc.print_row([average_results[key] for key in last_results_keys],
        colwidth=12)
    misc.print_row([std_results[key] for key in last_results_keys],
        colwidth=12)
